...
def run_abm(output_folder, NUM_LANDSCAPE_CELLS):
    ...
    for simulation_folder in simulation_folders:
        ...
        try:
            ...
            boundary_patches = gdal.Open(os.path.join(OUTPUT_FOLDER, "coverage_matrix_init/potential_coverage_matrix.tif")).ReadAsArray()
...
def make_regret_df(output_folder, MAX_GAME_STEPS, NUM_LANDSCAPE_CELLS, BUDGET_K):
    ...
    if len(game_steps) != len(defender_regret_values):
        ...
        output_filepath = os.path.join(OUTPUT_FOLDER, 'coverage_matrix_init/regret_df_FPL-UE_learning-memory-v1_1.csv')

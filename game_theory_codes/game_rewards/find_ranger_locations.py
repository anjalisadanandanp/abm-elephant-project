...
class optimise_ranger_locations():
    ...
    def __init__(self, num_rangers, ranger_visibility_radius, data_folder, output_folder):
    ...
    def plot_filtered_trajectories(self):
    ...
    def save_strategies_to_yaml(self, strategy, filename):
    ...
    def cost_function_kde_v1(self, x=None, grid_resolution=100, make_plots=False):
    ...
    def cost_function_kde_v2(self, x=None, grid_resolution=100, forest_penalty=1000, make_plots=False):
        ...
        all_lons_plantations = []
        all_lats_plantations = []

        for traj in self.trajectories:
            traj = traj[traj["landuse"] == 10]
            if len(traj) > 0:
                all_lons_plantations.extend(traj["longitude"].values)
                all_lats_plantations.extend(traj["latitude"].values)

        outProj, inProj =  Proj(init='epsg:4326'),Proj(init='epsg:3857')
        x, y = transform(inProj, outProj, all_lons_plantations, all_lats_plantations)                                             
        ...
        for pos in ranger_positions:
            ...
            landuse = data_LULC[py, px]
            if landuse != 7:  
                total_cost += forest_penalty 

        self.loss_history.append(total_cost)

        return total_cost

    def optimize(self, max_steps=1000):
        ...
        points = np.column_stack((all_lons_plantations, all_lats_plantations))
        kmeans = KMeans(n_clusters=self.num_rangers)
        kmeans.fit(points)
        ...
        )                           #methods = 'L-BFGS-B', "Nelder-Mead", "BFGS", "Powell"
        ...
        self.plot_filtered_trajectories()
        self.plot_trajectories_untill_ranger_intervention()
        ...
        self.save_strategies_to_yaml(strategy, os.path.join(self.output_folder, 'ranger_strategies_' + str(self.num_rangers) + 'rangers.yaml'))
...
if __name__ == "__main__":
    ...
    experiment_name = "vector_01-29-25__20-52"
    ...
    data_folder = os.path.join(os.getcwd(), "model_runs/ranger-deployment-within-plantations/", experiment_name, "without_rangers", starting_location, elephant_category, landscape_food_probability, 
    ...
    output_folder = os.path.join(os.getcwd(), "model_runs/ranger-deployment-within-plantations/", experiment_name, "guard_agent_placement_optimisation", "without_rangers")

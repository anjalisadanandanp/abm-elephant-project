...
class Elephant(GeoAgent):
    ...
    def next_step_to_move(self):
        ...
        return 
    #--------------------------------------------------------------------------------------------------
    def next_step_to_move_modified(self):
        """how the elephant agent moves from the current co-ordinates to the next"""
        ...
        if self.mode == "RandomWalk":
            ...
            if self.target_present == False:
                ...
                filter = self.return_feasible_direction_to_move_strategic_food()

                rand_num = self.model.random.uniform(0,1)  
                if rand_num <= self.model.prob_drink_water:
                    self.target_to_drink_water_modified(filter)    
                    self.radius_water_search = self.model.radius_water_search
                    self.target_name = "water"    

                else:
                    self.target_for_foraging_modified(filter)     
                    self.target_name = "food" 
            ...
            if self.target_present == False:
                filter = self.return_feasible_direction_to_move_strategic_water()
            ...
            # print("proximity to a water source:", self.model.water_proximity[self.ROW][self.COL]*33.33)
            if self.model.water_proximity[self.ROW][self.COL]*33.33 < 75:
                self.num_steps_thermoregulated += 1
    ...
    def return_feasible_direction_to_move_modified(self):
        ...
        return filter
    #-----------------------------------------------------------------------------------------------------
    def return_feasible_direction_to_move_strategic_food(self):
        ...
        return filter
    #-----------------------------------------------------------------------------------------------------
    def return_feasible_direction_to_move_strategic_water(self):
        ...
        #extract the food data
        water = np.array(self.water_memory)[self.ROW - radius//2:self.ROW + radius//2 + 1, self.COL - radius//2:self.COL + radius//2 + 1]
        ...
        #calculate the cost of movement in each direction as sum of the direction cells
        water_0 = sum(x for x in water[data == 1] if x > 0)
        water_1 = sum(x for x in water[data == 2] if x > 0)
        water_2 = sum(x for x in water[data == 3] if x > 0)
        water_3 = sum(x for x in water[data == 4] if x > 0)
        water_4 = sum(x for x in water[data == 5] if x > 0)
        water_5 = sum(x for x in water[data == 6] if x > 0)
        water_6 = sum(x for x in water[data == 7] if x > 0)
        water_7 = sum(x for x in water[data == 8] if x > 0)

        # num_water_cells_0 = sum(1 for x in water[data == 1] if x > 0)
        # num_water_cells_1 = sum(1 for x in water[data == 2] if x > 0)
        # num_water_cells_2 = sum(1 for x in water[data == 3] if x > 0)
        # num_water_cells_3 = sum(1 for x in water[data == 4] if x > 0)
        # num_water_cells_4 = sum(1 for x in water[data == 5] if x > 0)
        # num_water_cells_5 = sum(1 for x in water[data == 6] if x > 0)
        # num_water_cells_6 = sum(1 for x in water[data == 7] if x > 0)
        # num_water_cells_7 = sum(1 for x in water[data == 8] if x > 0)

        direction = [135, 90, 45, 0, 315, 270, 225, 180]

        slope_cost = [cost_0, cost_1, cost_2, cost_3, cost_4, cost_5, cost_6, cost_7]
        
        water_available = [water_0, water_1, water_2, water_3, water_4, water_5, water_6, water_7]
        
        # num_water_cells = [num_water_cells_0, num_water_cells_1, num_water_cells_2, num_water_cells_3, num_water_cells_4, num_water_cells_5, num_water_cells_6, num_water_cells_7]
        ...
        # for i, (x,y) in enumerate(zip(water_available, num_water_cells)):
        #     if (x > 0) and (y>0):
        #         water_available[i] = x/y
        #     else:
        #         food_available[i] = 0

        if max(water_available) > 0:
            water_available = [x/max(water_available) for x in water_available]
        else:
            water_available = [x for x in water_available]

        weight_water = 0.025
        weight_slope = 0.975

        water_available = [x*weight_water for x in water_available]
        slope_cost = [x*weight_slope for x in slope_cost]

        total_cost = [y-x for x, y in zip(water_available, slope_cost)] 
        # print("total cost: ", total_cost)
        ...
        ax[1].imshow(filter, cmap='viridis')
        ax[1].set_title("Filter")
        ax[2].imshow(water, cmap='viridis', vmin=0, vmax=1)
        ax[2].set_title("Water Cells")
        ax[3].imshow(distance_weight, cmap="twilight_shifted", vmin=0, vmax=1)
        ax[3].set_title("Distance Weight")
    ...
    def elephant_cognition(self):
        ...
        self.next_step_to_move_modified()
    ...
    def step(self):     
        ...
        try:
            ...
            print("Error in elephant cognition: ", e)

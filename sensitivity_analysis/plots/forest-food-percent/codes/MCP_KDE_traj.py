        ...
        for aggression in [0.2, 0.8]:
            for month_idx in [3, 8]:
                ...
                for prob_food_forest in [0.08, 0.09, 0.1, 0.11]:
                    ...
                    area_list_mcp.append(my_plot.MCP)
                    area_list_kde.append(my_plot.KDE)

                #plot the box plots
                fig, ax = plt.subplots()
                ax.boxplot(area_list_mcp, positions = [1, 2, 3, 4], widths = 0.5, patch_artist = True)
                ax.set_xlabel("Probability of food in forest")
                ax.set_ylabel("Area (sq. km)")
                ax.set_xticks([1, 2, 3, 4])
                ax.set_xticklabels(["0.08", "0.09", "0.1", "0.11"])
                plt.savefig("sensitivity_analysis/plots/forest-food-percent/images/MCP_food_val:" + str(food_val_forest) + "_agg:" + str(aggression) + "_mon:" + str(month_idx) + ".png", dpi=300, bbox_inches='tight')

                fig, ax = plt.subplots()
                ax.boxplot(area_list_kde, positions = [1, 2, 3, 4], widths = 0.5, patch_artist = True)
                ax.set_xlabel("Probability of food in forest")
                ax.set_ylabel("Area (sq. km)")
                ax.set_xticks([1, 2, 3, 4])
                ax.set_xticklabels(["0.08", "0.09", "0.1", "0.11"])
                plt.savefig("sensitivity_analysis/plots/forest-food-percent/images/KDE_food_val:" + str(food_val_forest) + "_agg:" + str(aggression) + "_mon:" + str(month_idx) + ".png", dpi=300, bbox_inches='tight')

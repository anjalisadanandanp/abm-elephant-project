...
#only elephant agent data is collected, human agent data not collected
...
class DataCollector:
    ...
    def _record_agents(self, model):
        ...
        #agent_records = map(get_reports, model.schedule.agents[0])
        agent_records = get_reports(model.schedule.agents[0])
    ...
    def get_agent_vars_dataframe(self):
        ...
        #print(rep_names)
        num_cols = len(["Step", "AgentID"] + rep_names)
        ...
        df = pd.DataFrame(data = np.array(list(all_records)).reshape(-1, num_cols), columns=["Step", "AgentID"] + rep_names)

...
class StackelbergTreeViz:
    ...
    def create_tree(self):
        ...
        for i in range(len(self.leader_payoffs)):
            ...
            for j in range(len(self.leader_payoffs[0])):
                ...
                self.pos[terminal_node] = (
                    leader_spacing * (i + 1)
                    + follower_spacing * (j - num_follower_strategies / 2),
                    0.3,
                )
    ...
    def solve_game(self, df, name="v1", output_path=None):
        num_leader_strategies = len(self.leader_payoffs)
        ...
        for leader_strategy in range(num_leader_strategies):
            ...
            print(
                f"Leader strategy {leader_strategy}: Follower's best response is {best_response}"
            )
            follower_best_responses[leader_strategy] = best_response

        leader_values = [
            self.leader_payoffs[i][follower_best_responses[i]]
        ...
        optimal_path = [
            ...
            (
                f"F{optimal_leader_strategy}",
                f"T{optimal_leader_strategy}{optimal_follower_response}",
            ),
        ]

        optimal_row = df[
            (df["strategy"] == optimal_leader_strategy + 1)
            & (df.index == int(optimal_follower_response))
        ]

        best_strategy = optimal_row
        best_strategy.to_csv(
            os.path.join(output_path, "best_strategy_" + name + ".csv"), index=False
        )
    ...
    def draw_tree(self, df, highlight_solution=True, name="v1", output_path=None):
        self.create_tree()
        plt.figure(figsize=(30, 4))

        nx.draw_networkx_nodes(
            self.G, self.pos, node_color=self.node_colors, node_size=1000
        )

        if highlight_solution:
            optimal_path = self.solve_game(df, name, output_path)
            edge_colors = [
                "red" if edge in optimal_path else "gray" for edge in self.G.edges()
            ]
        else:
        ...
        for node in self.G.nodes():
            if node == "L":
                ...
                labels[node] = (
                    f"({self.leader_payoffs[i][j]},{self.follower_payoffs[i][j]})"
                )
        ...
        if output_path == None:
            plt.savefig(
                "trajectory_analysis/stackelberg_game_example/game_tree_"
                + name
                + ".png",
                dpi=300,
                bbox_inches="tight",
            ...
            plt.savefig(
                os.path.join(output_path, "game_tree_" + name + ".png"),
                dpi=300,
                bbox_inches="tight",
            )

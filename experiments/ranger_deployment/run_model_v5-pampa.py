...
from mesageo_elephant_project.elephant_project.model.abm_model_HEC_v4 import (
    batch_run_model,
)
from experiments.ranger_deployment.experiment_names import FancyNameGenerator
from trajectory_analysis.stackelberg_game_example.SSG_solve_v1 import StackelbergTreeViz
...
class Experiment:
    ...
    def create_ranger_strategies(
        ...
        def initialize_road_network(make_plots=True):
            ...
            center_lon, center_lat = transform(
                inProj, outProj, longitude_center, latitude_center
            )

            latextend, lonextend = [
                center_lat - delta[area_size],
                center_lat + delta[area_size],
            ], [center_lon - delta[area_size], center_lon + delta[area_size]]
            ...
            self.LON_MIN, self.LAT_MIN = transform(
                inProj, outProj, self.LON_MIN_epsg3857, self.LAT_MIN_epsg3857
            )
            self.LON_MAX, self.LAT_MAX = transform(
                inProj, outProj, self.LON_MAX_epsg3857, self.LAT_MAX_epsg3857
            )
            ...
            self.road_network = ox.graph.graph_from_bbox(
                north,
                south,
                east,
                west,
            ...
            self.nodes_proj, self.edges_proj = ox.graph_to_gdfs(
                self.road_network, nodes=True, edges=True
            )
        ...
        def visualize_road_network(nodes_proj, edges_proj):
            ...
            plt.savefig(
                os.path.join(pathlib.Path(data_folder), "road_network_graph.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        def create_road_matrix(edges_proj, row_size, col_size, make_plots=True):
            ...
            for _, edge in edges_proj.iterrows():
                ...
                for i in range(len(coords) - 1):
                    ...
                    matrix_x = ((xs - min_x) / (max_x - min_x) * (row_size - 1)).astype(
                        int
                    )
                    matrix_y = ((ys - min_y) / (max_y - min_y) * (col_size - 1)).astype(
                        int
                    )
            ...
            if make_plots:
                ...
                ax.axis("off")
                plt.savefig(
                    os.path.join(pathlib.Path(data_folder), "road_matrix.png"),
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
        ...
        def calculate_proximity_map(
            landscape_matrix, target_class, name, make_plots=True, save=False
        ):
            ...
            if save:
                ...
                loc = os.path.join(
                    pathlib.Path(data_folder), "proximity_to_" + name + ".tif"
                )
            ...
            if make_plots:
                ...
                img = ax.imshow(
                    ...
                    cmap="coolwarm",
                    vmin=0,
                    vmax=np.max(proximity_matrix),
                )
                ax.axis("off")
                cbar = fig.colorbar(
                    img, ax=ax, orientation="vertical", fraction=0.046, pad=0.04
                )
                cbar.ax.tick_params(labelsize=8)
                plt.savefig(
                    os.path.join(
                        pathlib.Path(data_folder), "proximity_to_" + name + ".png"
                    ),
                    dpi=300,
                    bbox_inches="tight",
                )
        ...
        def get_inverse_proximity_map(plantation_proximity_map, name, make_plots=True):
            ...
            plantation_proximity_inverse = (
                np.max(plantation_proximity_map) - plantation_proximity_map
            )
            ...
            if make_plots:
                ...
                img = ax.imshow(
                    ...
                    cmap="coolwarm",
                    vmin=0,
                    vmax=np.max(plantation_proximity_inverse),
                )
                ax.axis("off")
                cbar = fig.colorbar(
                    img, ax=ax, orientation="vertical", fraction=0.046, pad=0.04
                ...
                plt.savefig(
                    os.path.join(
                        ...
                        "proximity_to_" + name + "_inverse.png",
                    ),
                    dpi=300,
                    bbox_inches="tight",
                )
        ...
        def get_plantation_forest_fringe(lulc_matrix, distance=1000, make_plots=True):
            ...
            plantation_forest_fringe = distance_transform_edt(
                forest_mask_inverse, return_distances=True
            )
            ...
            for row in range(plantation_forest_fringe.shape[0]):
                for col in range(plantation_forest_fringe.shape[1]):
                    if (
                        plantation_forest_fringe[row, col] > 0
                        and plantation_forest_fringe[row, col] * 33.33 <= distance
                        and plantation_mask[row, col] == 1
                    ):
                        plantation_forest_fringe_mask[row, col] = (
                            plantation_forest_fringe[row, col]
                        )
            ...
            if make_plots:
                fig, ax = plt.subplots(figsize=(6.8, 6.8))
                img = ax.imshow(
                    plantation_forest_fringe_mask,
                    cmap="coolwarm",
                    vmin=np.min(plantation_forest_fringe_mask),
                    vmax=np.max(plantation_forest_fringe_mask),
                )
                ax.axis("off")
                cbar = fig.colorbar(
                    img, ax=ax, orientation="vertical", fraction=0.046, pad=0.04
                ...
                plt.savefig(
                    os.path.join(
                        pathlib.Path(data_folder), "plantation_forest_fringe.png"
                    ),
                    dpi=300,
                    bbox_inches="tight",
                )
        ...
        def cost_function_kde_v1(x=None, grid_resolution=100):
            ...
            if self.elephant_kde is None:
                ...
                self.lat_grid, self.lon_grid = np.mgrid[
                    min(y) : max(y) : grid_resolution * 1j,
                    min(x) : max(x) : grid_resolution * 1j,
                ]
                positions = np.vstack([self.lon_grid.ravel(), self.lat_grid.ravel()])
                elephant_density = np.reshape(
                    self.elephant_kde(positions).T, self.lat_grid.shape
                )
                self.elephant_density = elephant_density / elephant_density.max()

            ranger_coverage = np.zeros_like(self.elephant_density)

            ranger_x, ranger_y = transform(
                inProj, outProj, ranger_positions[:, 0], ranger_positions[:, 1]
            )
            ...
            for i, (rx, ry) in enumerate(zip(ranger_x, ranger_y)):
                ...
                distances = np.sqrt(
                    (self.lon_grid - rx) ** 2 + (self.lat_grid - ry) ** 2
                )
            ...
            uncovered_score = np.sum(
                (1 - ranger_coverage) * self.elephant_density
            ) / np.sum(self.elephant_density)
        ...
        def optimize(
            starting_positions="kmeans_all_points", max_steps=100, strategy=None
        ):

            initial_positions = []

            all_lons = np.concatenate(
                [traj["longitude"].values for traj in self.all_trajectories]
            )
            all_lats = np.concatenate(
                [traj["latitude"].values for traj in self.all_trajectories]
            )
            ...
            if starting_positions == "kmeans_all_points":
                ...
                kmeans = KMeans(
                    n_clusters=model_params_all["num_guards"], random_state=42
                )
                kmeans.fit(points)
            ...
            bounds = [(lon_min, lon_max), (lat_min, lat_max)] * model_params_all[
                "num_guards"
            ]
            ...
            ax.annotate(
                f"Minimum Loss: {min(self.loss_history):.2f}",
            ...
            plt.savefig(
                os.path.join(data_folder, "optimization_loss" + str(strategy) + ".png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        def sample_random_ranger_locations(
            preferences, num_strategies=1, make_plots=True
        ):
            ...
            for strategy in range(num_strategies):
                ...
                if valid_points:
                    ...
                    sampled_indices = np.random.choice(
                        len(valid_points), num_rangers, replace=False
                    )
                ...
                if make_plots:
                    ...
                    for ranger in ranger_locations:
                        ...
                        ax.scatter(
                            x_new, y_new, 50, marker="x", color="black", zorder=2
                        )

                    plt.savefig(
                        os.path.join(
                            pathlib.Path(data_folder),
                            "ranger_locations__" + str(strategy) + ".png",
                        ),
                        dpi=300,
                        bbox_inches="tight",
                    )
            ...
            for i, ranger_pos in enumerate(ranger_location):
                ...
                for traj in self.all_trajectories:
                    ...
                    for idx, (lon, lat) in enumerate(
                        zip(traj["longitude"], traj["latitude"])
                    ):
                        distance = np.sqrt(
                            (lon - ranger_pos[0]) ** 2 + (lat - ranger_pos[1]) ** 2
                        )
        ...
        def ranger_strategies_v1(num_strategies=1, make_plots=True):
            ...
            for strategy in range(num_strategies):
                ...
                ranger_locations, total_cost, convergence = optimize(
                    starting_positions="kmeans_all_points",
                ...
                if make_plots:
                    ...
                    lat_grid, lon_grid = np.mgrid[
                        min(y) : max(y) : 100j, min(x) : max(x) : 100j
                    ]
        ...
        def get_forest_fringe(distance=1000, make_plots=True, save=False):
            ...
            road_matrix = create_road_matrix(
                self.edges_proj, 1069, 1070, make_plots=make_plots
            )
            road_proximity_map = calculate_proximity_map(
                landscape_matrix=road_matrix,
                target_class=1,
                name="roads",
                make_plots=make_plots,
                save=save,
            )
            road_proximity_inverse = get_inverse_proximity_map(
                road_proximity_map, "roads", make_plots=make_plots
            )

            LULC = read_LULC_map()

            plantation_proximity_map = calculate_proximity_map(
                landscape_matrix=LULC,
                target_class=10,
                name="plantations",
                make_plots=make_plots,
                save=save,
            )
            plantation_proximity_inverse = get_inverse_proximity_map(
                plantation_proximity_map, "plantations", make_plots=make_plots
            )

            river_proximity_map = calculate_proximity_map(
                landscape_matrix=LULC,
                target_class=5,
                name="rivers",
                make_plots=make_plots,
            ...
            river_proximity_inverse = get_inverse_proximity_map(
                river_proximity_map, "rivers", make_plots=make_plots
            )

            ebf_proximity_map = calculate_proximity_map(
                landscape_matrix=LULC,
                target_class=15,
            ...
            ebf_proximity_inverse = get_inverse_proximity_map(
                ebf_proximity_map, "ebf", make_plots=make_plots
            )

            plantation_forest_distances = get_plantation_forest_fringe(
                LULC, distance=distance, make_plots=make_plots
            )
        ...
        if step == 1:
            ...
            ranger_locations = sample_random_ranger_locations(
                preferences=plantation_forest_distances,
                num_strategies=4,
                make_plots=True,
            )

        else:
            ranger_locations = ranger_strategies_v1(
                num_strategies=1, make_plots=make_plots
            )

        with open(
            os.path.join(
                pathlib.Path(data_folder),
                "ranger_strategies_" + str(num_rangers) + "rangers.yaml",
            ),
            "w",
        ) as file:
    ...
    def evaluate_trajectories(
        self, data_folder, model_params, step, strategy, num_rangers, make_plots=False
    ):
        ...
        def read_experiment_data(yaml_file_path):
            ...
            yaml_file = os.path.join(
                yaml_file_path, "ranger_strategies_" + str(num_rangers) + "rangers.yaml"
            )
            ...
            try:
                ...
                for experiment in data:
                    experiment["ranger_locations"] = np.array(
                        experiment["ranger_locations"]
                    )
                    experiment["ranger_payoffs"] = np.array(
                        experiment["ranger_payoffs"]
                    )
            ...
            for data in trajectories:
                ...
                for ranger in ranger_locations:
                    ...
                    for lon, lat in zip(longitude, latitude):
                        distance = np.sqrt(
                            (lon - ranger_lon) ** 2 + (lat - ranger_lat) ** 2
                        )
        ...
        def plot_filtered_trajectories(
            intersecting_trajs, non_intersecting_trajs, ranger_locations
        ):
            ...
            for data in intersecting_trajs:
                longitude, latitude = transform(
                    inProj, outProj, data["longitude"], data["latitude"]
                )
            ...
            plt.savefig(
                os.path.join(
                    pathlib.Path(data_folder),
                    "trajectories_with_ranger_locations__numrangers"
                    + str(model_params["num_guards"])
                    + "_rangervisibility"
                    + str(model_params["ranger_visibility_radius"])
                    + "m_v2.png",
                ),
                dpi=300,
                bbox_inches="tight",
        ...
        def find_first_visible_entry(trajectory, ranger_locations):
            ...
            for idx, row in trajectory.iterrows():
                for ranger_location in ranger_locations:
                    distance = (
                        (row["longitude"] - ranger_location[0]) ** 2
                        + (row["latitude"] - ranger_location[1]) ** 2
                    ) ** 0.5
        ...
        def calculate_landuse_time(trajectory):
            ...
            assert total_time == len(
                trajectory
            ), "Total time steps don't match trajectory length"
        ...
        def plot_trajectories_untill_ranger_intervention(
            ...
            for i, traj in enumerate(trajectories):
                ...
                ax.plot(x_new, y_new, "b-", alpha=0.4, label="Trajectory")

                ax.scatter(x_new[0], y_new[0], 25, marker="o", color="black", zorder=2)
                ax.scatter(
                    x_new[-1], y_new[-1], 25, marker="^", color="black", zorder=2
                )
            ...
            plt.savefig(
                os.path.join(
                    pathlib.Path(data_folder),
                    "trajectories_until_ranger_proximity__rangervisibility"
                    + str(model_params["ranger_visibility_radius"])
                    + "m_v1.png",
                ),
                dpi=300,
        ...
        def calculate_average_plantation_usage(trajectories):
            ...
            print(
                "Number of intersecting trajectories:",
                len(intersecting_trajs),
                "Number of non-intersecting trajectories:",
                len(non_intersecting_trajs),
                "Average Plantation Usage:",
                average_usage,
            )
        ...
        def sort_trajectories(
            ...
            pairs = list(zip(trajectory_payoffs, trajectories, trajectory_names))
            sorted_pairs = sorted(pairs, key=lambda x: x[0], reverse=True)

            trajs_with_best_payoffs = [traj for _, traj, x in sorted_pairs]
            payoffs = [payoff for payoff, _, x in sorted_pairs]
            trajectory_names = [x for _, _, x in sorted_pairs]
            ...
            if make_plots:
                ...
                outProj, inProj = Proj(init="epsg:4326"), Proj(init="epsg:3857")
                LON_MIN, LAT_MIN = transform(
                    inProj, outProj, xmin, ymax + yres * col_size
                )
                LON_MAX, LAT_MAX = transform(
                    inProj, outProj, xmin + xres * row_size, ymax
                )
                ...
                payoff_colors = plt.cm.RdYlGn_r(
                    (trajectory_payoffs - min(trajectory_payoffs))
                    / (max(trajectory_payoffs) - min(trajectory_payoffs))
                )

                for data, payoff, color in zip(
                    trajs_with_best_payoffs, payoffs, payoff_colors
                ):
                ...
                sm = plt.cm.ScalarMappable(
                    ...
                    norm=plt.Normalize(
                        min(trajectory_payoffs), max(trajectory_payoffs)
                    ),
                )
            ...
            return trajs_with_best_payoffs, payoffs, trajectory_names

        def assign_payoffs_v1(trajectories, ranger_locations, cost):

            trajectory_payoffs = []

            intersecting_trajs, non_intersecting_trajs = (
                filter_trajectories_in_ranger_radius(trajectories, ranger_locations)
            )
            ...
            for i, traj in enumerate(trajectories):
                ...
                if is_df_in_list(traj, intersecting_trajs):
                    for plantation_use, food in zip(
                        plantation_use_time, daily_food_consumed
                    ):
                        if plantation_use > 0:
                    ...
                    for plantation_use, food in zip(
                        ...
                        if (
                            plantation_use > 0
                            and food >= traj["daily_dry_matter_intake"].unique()[0]
                        ):
                            trajectory_fitness += cost
        ...
        for folder in folders:
            ...
            agent_data = pd.read_csv(
                os.path.join(data_folder, folder, "output_files/agent_data.csv")
            )
            agent_data = agent_data[agent_data["AgentID"] == "bull_0"].reset_index(
                drop=True
            )

            trajectories.append(agent_data)
            trajectory_names.append(folder)

            self.all_trajectories.append(agent_data)

            ranger_strategy = read_experiment_data(
                os.path.join(data_folder, folder, "env/ranger_strategy/")
            )
            converged_experiments = [
                exp for exp in ranger_strategy if exp["convergence"]
            ]
        ...
        trajectory_payoffs = assign_payoffs_v1(
            trajectories, ranger_locations, cost=0.05
        )

        sorted_trajectories, sorted_payoffs, trajectory_names = sort_trajectories(
            trajectory_payoffs,
            trajectories,
            trajectory_names,
            make_plots=True,
        ...
        return sorted_trajectories, sorted_payoffs, trajectory_names

    def evaluate_range_strategies(
        self,
        sorted_trajectories,
        data_folder,
        model_params,
        ...
        for traj in sorted_trajectories:
            landuse = calculate_landuse_time(traj)[7]
            payoffs.append(-landuse)

        return payoffs

    def create_payoff_matrices(self, payoff_df_path, make_plots=True):

        df = pd.read_csv(payoff_df_path)

        num_ranger_strategies = df["strategy"].nunique()
        num_elephant_strategies = df["trajectory"].nunique()

        elephant_matrix = np.zeros((num_ranger_strategies, num_elephant_strategies))
        ranger_matrix = np.zeros((num_ranger_strategies, num_elephant_strategies))

        for _, row in df.iterrows():
            strategy_idx = int(row["strategy"]) - 1
            trajectory_idx = int(_)
            elephant_matrix[strategy_idx][trajectory_idx] = row["elephant_payoff"]
            ranger_matrix[strategy_idx][trajectory_idx] = row["ranger_payoff"]

        if make_plots:
            output_path = os.path.join(
                pathlib.Path(payoff_df_path).parent, "payoff_matrices.png"
            )
            self.plot_payoff_matrices(elephant_matrix, ranger_matrix, output_path)

        return elephant_matrix, ranger_matrix

    def plot_payoff_matrices(self, elephant_matrix, ranger_matrix, output_path=None):

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))

        sns.heatmap(elephant_matrix, annot=True, cmap="Blues", ax=ax1, fmt=".2f")
        ax1.set_title("Elephant Payoffs")
        ax1.set_xlabel(f"Elephant Strategy (1-{elephant_matrix.shape[1]})")
        ax1.set_ylabel(f"Ranger Strategy (1-{elephant_matrix.shape[0]})")

        sns.heatmap(ranger_matrix, annot=True, cmap="Reds", ax=ax2, fmt=".2f")
        ax2.set_title("Ranger Payoffs")
        ax2.set_xlabel(f"Elephant Strategy (1-{ranger_matrix.shape[1]})")
        ax2.set_ylabel(f"Ranger Strategy (1-{ranger_matrix.shape[0]})")
    ...
    def run_experiment(self, send_notification):
        ...
        for model_params in param_dicts:
            ...
            threshold_food_derivation_days = (
                ...
                + str(model_params["threshold_days_of_food_deprivation"])
            )
            threshold_water_derivation_days = (
                "threshold_days_of_water_deprivation-"
                + str(model_params["threshold_days_of_water_deprivation"])
            )
            ...
            for step in range(1, optimisation_args["max_optimisation_steps"] + 1):
                ...
                for strategy in range(1, optimisation_args["num_strategies"] + 1):
                    ...
                    output_folder = os.path.join(
                        ...
                        str(model_params["year"]),
                        str(model_params["month"]),
                        "guard_agent_placement_optimisation",
                        "step-" + str(step),
                        "strategy-" + str(strategy),
                    )
                    ...
                    self.create_ranger_strategies(
                        output_folder,
                        num_rangers=optimisation_args["num_rangers"],
                        step=step,
                        num_strategies=1,
                        make_plots=True,
                    )
                    ...
                    run_model(
                        experiment_name, model_params, data_folder, step, strategy
                    )
                    # ------------run agent based model----------------#

                    # -----evaluate the effectiveness of the trajectories and ranger placement-----#
                    sorted_trajectories, sorted_payoffs, trajectory_names = (
                    ...
                    ranger_payoffs = self.evaluate_range_strategies(
                        sorted_trajectories,
                        data_folder,
                        model_params,
                        step,
                    ...
                    for i, (
                        ...
                        zip(trajectory_names, sorted_payoffs, ranger_payoffs)
                    ):
                        payoff_df_data.append(
                            [
                                step,
                                strategy,
                                trajectory_name,
                ...
                payoff_df.to_csv(
                    os.path.join(pathlib.Path(output_folder).parent, "payoff_df.csv")
                )

                # payoff_df = pd.read_csv(os.path.join(pathlib.Path(output_folder).parent, "payoff_df.csv"))
                elephant_matrix, ranger_matrix = self.create_payoff_matrices(
                    os.path.join(pathlib.Path(output_folder).parent, "payoff_df.csv"),
                    make_plots=True,
                )

                elephant_matrix = np.round(elephant_matrix, 2)
                ranger_matrix = np.round(ranger_matrix, 2)

                viz = StackelbergTreeViz(ranger_matrix, elephant_matrix)
                viz.draw_tree(
                    payoff_df,
                    highlight_solution=True,
                    name="v1",
                    output_path=os.path.join(pathlib.Path(output_folder).parent),
                )

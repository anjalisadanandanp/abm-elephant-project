...
def run_abm_and_return_cost(model_params, ranger_locations, experiment_name):
    ...
    attacker_rewards_df = pd.read_csv("game_theory_codes/FPL-UE/outputs/attacker_rewards_penalties.csv")
    defender_rewards_df = pd.read_csv("game_theory_codes/FPL-UE/outputs/defender_rewards_penalties.csv")
...
def plot_optimization_results(history):
   ...
   plt.grid(True)
   plt.savefig('game_theory_codes/FPL-UE/outputs/optimization_results.png', bbox_inches='tight', dpi=300)
   plt.close()
...
if __name__ == "__main__":
    ...
    experiment_name = "ranger-deployment-within-plantations-FPL-UE/" + run_name

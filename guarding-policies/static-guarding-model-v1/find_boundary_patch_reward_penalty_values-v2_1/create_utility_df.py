...
def find_food_value_association():
    ...
    for max_cells_per_group in tqdm([5, 10, 15, 20, 25, 30, 35, 40, 45, 50]):
        ...
        boundary_patches = gdal.Open(os.path.join("guarding-policies/static-guarding-model-v1/create-strategy-matrix-v2/boundary_raster_discretised_" + str(int(max_cells_per_group*30)) + "m/boundary_raster_discretised.tif")).ReadAsArray()

        save_folder = os.path.join("guarding-policies/static-guarding-model-v1/find_boundary_patch_reward_penalty_values-v2_1/boundary_raster_discretised_" + str(int(max_cells_per_group*30)) + "m")

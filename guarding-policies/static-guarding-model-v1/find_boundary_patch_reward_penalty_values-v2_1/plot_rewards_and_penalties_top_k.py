...
def make_plots(output_folder, output_folder_raster):
    ...
    df = pd.read_csv(os.path.join(output_folder, "boundary_patch_reward_penalty_matrix.csv"))

    sorted_df = df.sort_values(by='reward', ascending=False)

    reward_df = sorted_df.head(k)

    print("top k boundary patches with highest rewards:", reward_df)
    ...
    plt.savefig(os.path.join(output_folder, "boundary_patches_with_rewards_top" + str(k) + "_.png"), dpi=750, bbox_inches='tight')
...
for max_cells_per_group in tqdm([5, 10, 15, 20, 25, 30, 35, 40, 45, 50]):
    ...
    make_plots(output_folder="guarding-policies/static-guarding-model-v1/find_boundary_patch_reward_penalty_values-v2_1/boundary_raster_discretised_" + str(int(max_cells_per_group*30)) + "m", output_folder_raster="guarding-policies/static-guarding-model-v1/create-strategy-matrix-v2/boundary_raster_discretised_" + str(int(max_cells_per_group*30)) + "m", )
...
class Elephant(GeoAgent):
    ...
    def update_danger_to_life(self):
        ...
        if np.any(coverage_matrix > 0):    
            ...
            targets = gdal.Open(os.path.join(self.model.path_to_defender_coverage_matrices, "defender_coverage_matrix.tif")).ReadAsArray()

            self.target_attacked = targets[self.attacked_target_row][self.attacked_target_col]
...
class conflict_model(Model):
    ...
    def __init__(self,
        ...
        self.model_day = 0
        self.path_to_defender_coverage_matrices = folder
    ...
    def create_trajectory_shapefile(self, agent_id, longitude, latitude):
        ...
        return  
    #----------------------------------------------------------------------------------------------------
    def update_defender_coverage_matrix(self, path):

        coverage_matrix_path = os.path.join(path, "defender_coverage_matrix.tif")
        ...
        target_ids = np.unique(potential_coverage_matrix)
        target_ids = target_ids[target_ids != 0]  # Exclude zero if it's not a target ID

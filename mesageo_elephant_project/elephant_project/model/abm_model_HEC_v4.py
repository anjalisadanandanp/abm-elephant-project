...
class Elephant(GeoAgent):
    ...
    def next_step_to_move_v1(self):
        ...
        return 
    #--------------------------------------------------------------------------------------------------
    def next_step_to_move_v2(self):
        """how the elephant agent moves from the current co-ordinates to the next"""

        self.mode = self.current_mode_of_the_agent_v2()   
    ...
    def update_danger_to_life(self):
        ...
        if len(self.conflict_neighbor) > 0:    
            ...
            return 
    #--------------------------------------------------------------------------------------------------
    def update_danger_to_life_v2(self):
        """Update danger_to_life"""

        if self.model.DETERRENT_POLICY[self.ROW][self.COL] > 0:
            self.danger_to_life = True
            self.conflict_with_humans = True 
            return         
    ...
    def current_mode_of_the_agent(self):
        ...
        return mode
    #-------------------------------------------------------------------------------------------------
    def current_mode_of_the_agent_v2(self):
        """function returns the mode of the agent depending on its energy levels and interaction with human agents"""
    ...
    def plot_stepwise_neighborhood_matrices(self, row_start, row_end, col_start, col_end, target_name, filter, target_x, target_y):
        ...
        ranger_coverage = np.array(self.model.ranger_coverage)[row_start:row_end, col_start:col_end]
        img7 = ax[6].imshow(ranger_coverage, cmap='coolwarm', vmin=0, vmax=1)
        ax[6].set_xticks([])
        ax[6].set_yticks([])
        plt.colorbar(img7, ax=ax[6], orientation='vertical', shrink=0.5)
        ...
        ax[6].scatter(y - col_start, x - row_start, color='green', s=25, marker='s')
        ...
        ax[6].scatter(target_y - col_start, target_x - row_start, color='red', s=50, marker='x')
    ...
    def target_for_escape_v2(self, filter):
        ...
        for i in range(row_start, row_end):
            for j in range(col_start, col_end):
                ...
                elif self.model.LANDUSE[i][j] == 15 and filter[i - row_start][j - col_start] == 1 and self.model.ranger_coverage[i,j] < 0.5:
                    coord_list.append([i, j])
...
class Guard_agents(Humans):
    ...
    def find_ranger_coverage(self):
        ...
        fid = os.path.join(self.model.folder_root, "env", "LULC.tif")

        ds = gdal.Open(fid)
        row_size, col_size = ds.ReadAsArray().shape

        lat_grid, lon_grid = np.mgrid[self.model.LAT_MIN_epsg3857:self.model.LAT_MAX_epsg3857:row_size*1j, 
                                      self.model.LON_MIN_epsg3857:self.model.LON_MAX_epsg3857:col_size*1j]

        ranger_coverage = np.zeros_like(ds.ReadAsArray())
        
        for i, ranger in enumerate(self.model.ranger_locations):

            distances = np.sqrt((lon_grid - ranger[0])**2 + (lat_grid - ranger[1])**2)
            coverage = np.exp(-0.5 * (distances / self.model.ranger_visibility_radius)**2)
            ranger_coverage = np.maximum(ranger_coverage, coverage)

        self.model.ranger_coverage = np.flipud(ranger_coverage)
        fig, ax = plt.subplots(figsize=(6.8, 6.8))
        map = Basemap(llcrnrlon= self.model.LON_MIN,llcrnrlat=self.model.LAT_MIN,urcrnrlon=self.model.LON_MAX,urcrnrlat=self.model.LAT_MAX, epsg=4326, resolution='l')
        img = map.imshow(np.flipud(self.model.ranger_coverage), 
                        cmap='coolwarm', 
                        alpha=0.75,  
                        extent=[self.model.LON_MIN, self.model.LON_MAX, self.model.LAT_MIN, self.model.LAT_MAX],
                        zorder=1)
        
        plt.colorbar(img, ax=ax, orientation='vertical', shrink=0.5)

        for ranger in self.model.ranger_locations:
            outProj, inProj =  Proj(init='epsg:4326'),Proj(init='epsg:3857') 
            ...
            radius = self.model.ranger_visibility_radius/(111*1000)  
            circle = plt.Circle((x_new, y_new), radius, facecolor='purple', fill=True, alpha=0.5, edgecolor='black', linewidth=1)
            ax.add_artist(circle)

        plt.savefig(os.path.join(folder, self.model.now, "output_files", 'ranger_coverages.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
class conflict_model(Model):
    ...
    def __init__(self,
        ...
        self.DETERRENT_POLICY = self.DETERRENT_POLICY()
        #-------------------------------------------------------------------
    ...
    def initialize_human_agents(self):
        ...
        for k in range(0, len(self.ranger_locations)):
            ...
            newagent.mode = None
            newagent.fitness = None
            newagent.daily_dry_matter_intake = None
            newagent.food_consumed = None
            newagent.visit_water_source = None
            newagent.target_present = None
            newagent.target_lon = None
            newagent.target_lat = None
            newagent.distance_to_target = None
            newagent.target_name = None
            newagent.num_days_water_source_visit = None
            newagent.num_days_food_depreceation = None
            newagent.num_thermoregulation_steps = None
            newagent.num_steps_thermoregulated = None
            newagent.current_proximity_to_plantations = None
            newagent.current_proximity_to_water_sources = None
            self.grid.add_agents(newagent)
            self.schedule.add(newagent)
